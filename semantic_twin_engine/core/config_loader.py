...
class EntityConfig(BaseModel):
    ...
    legacy_identity: list[str] = Field(
        default_factory=list, description="Keywords representing current perception"
    )
    strategic_target: list[str] = Field(
        default_factory=list, description="Keywords representing target perception"
    )
...
class AnchorConfig(BaseModel):
    ...
    entities: list[str] = Field(
        default_factory=list, description="Reference entity names for comparison"
    )
    concepts: list[str] = Field(
        default_factory=list, description="Reference concepts for analysis"
    )
...
class OpenAIConfig(BaseModel):
    ...
    temperature: float = Field(
        default=0.0, ge=0.0, le=2.0, description="Temperature (0 for deterministic)"
    )
...
class OutputConfig(BaseModel):
    ...
    directory: Path = Field(
        default=Path("./output"), description="Output directory path"
    )
    format: str = Field(default="json", pattern="^(json|yaml)$")
    include_metadata: bool = Field(
        default=True, description="Include execution metadata in output"
    )
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    and validates the settings.yaml configuration. If no config_path is provided,
    it will try to load from environment variable SEMANTIC_TWIN_COMPANY,
    mapping it to the appropriate config file.

    Args:
        config_path: Optional path to settings.yaml. Defaults to
            ./config/{COMPANY}_settings.yaml based on SEMANTIC_TWIN_COMPANY
            environment variable (defaults to PMI).
    ...
    if config_path is None:
        # Map company names to config files
        company = os.getenv("SEMANTIC_TWIN_COMPANY", "PMI").strip().upper()
        company_config_map = {
            "PMI": "PMI_settings.yaml",
            "PHILIP MORRIS": "PMI_settings.yaml",
            "PHILIP MORRIS INTERNATIONAL": "PMI_settings.yaml",
            "TE": "TE_settings.yaml",
            "TOTALENERGIES": "TE_settings.yaml",
        }

        config_filename = company_config_map.get(company, "settings.yaml")
        config_path = Path(__file__).parent.parent / "config" / config_filename

        logger.info(
            f"Loading configuration for company: {company} from {config_path.name}"
        )
    ...
    if raw_config is None:
        raise ConfigurationError(
            "Configuration file is empty", details={"path": str(config_path)}
        )
